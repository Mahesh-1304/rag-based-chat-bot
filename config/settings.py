...
class Settings(BaseSettings):
    ...
    # Model path (should be in MODELS_DIR)
    MODEL_PATH: Optional[Path] = None  # Load from .env if needed
    
    # ============ Embedding Model ============
    EMBEDDING_MODEL_NAME: str = "all-MiniLM-L6-v2"
    EMBEDDING_VECTOR_DIM: int = 384  # Output dimension for MiniLM
    ...
    # ============ Vector Store ============
    VECTOR_STORE_TYPE: str = "hnsw"  # "flat" | "hnsw" | "ivf"
    HNSW_M: int = 32  # Number of neighbors for HNSW
    IVF_NLIST: int = 100  # Number of clusters for IVF
    
    # ============ Retriever ============
    RETRIEVER_TOP_K: int = 3
    RETRIEVER_SCORE_THRESHOLD: float = 0.5  # Minimum similarity score
    
    # ============ LLM Parameters ============
    LLM_TEMPERATURE: float = 0.0  # No creativity - factual only
